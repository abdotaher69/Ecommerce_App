...
class ProductProvider with ChangeNotifier{
  List<ProductModel> products=[];
   ...
   List<ProductModel>  getProductsByCategory(String category){
     return products.where((element) => element.productCategory.toLowerCase().contains(category.toLowerCase())).toList();
   }
   List<ProductModel>  searchQuery({required String searchText,required List<ProductModel>passedList}){
     return passedList.where((element) =>
         element.productTitle.toLowerCase().contains(searchText.toLowerCase())).toList();
   }
  final productsDB =  FirebaseFirestore.instance.collection('products');

  Future<List<ProductModel>> fetchProducts() async {
     try{
     await productsDB.orderBy('createdAt',descending: true).get().then((snapshot) {
